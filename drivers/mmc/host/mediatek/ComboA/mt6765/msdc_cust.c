...
void msdc_sd_power_off(void)
{
	...
	if (host) {
		...
		host->mmc->ios.vtf_ocp = true;
		host->mmc->ios.ocp_gpio = mmc_gpio_ocp_status(host->mmc);
		pr_warn("%s vtf_ocp = %s,gpio_cd = %d.\n", __func__,
			"true", host->mmc->ios.ocp_gpio);
...
int msdc_of_parse(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	if (of_get_property(np, "fast_power_off", NULL)) {
		/* PD1732F_EX should not enable this feature.
		 * PD1732DF_EX should enable it.
		 * */
		char *version = get_board_version();
		pr_notice("[msdc%d]: board_info %c\n", mmc->index, *(version + 2));
		if (*(version + 2) == '0') {
